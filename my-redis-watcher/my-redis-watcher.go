...
//
func NewWatcher(addr string, option WatcherOptions) (*WatcherEx1, error) {
	option.Addr = addr
	initConfig(&option)
	w := &WatcherEx1{
...
func (w *WatcherEx1) initConfig(option WatcherOptions) error {
	var err error
...
func NewPublishWatcher(addr string, option WatcherOptions) (*WatcherEx1, error) {
	option.Addr = addr
	w := &WatcherEx1{
		pubClient: rds.NewClient(&option.Options),
		ctx:       context.Background(),
...
func (w *WatcherEx1) SetUpdateCallback(callback func(string)) error {
	w.l.Lock()
	w.callback = callback
	w.l.Unlock()
	return nil
...
func (w *WatcherEx1) UpdateNamedPolicy(sec string,ptype string, p1 []string, p2 []string)error{
	return w.logRecord(func() error {
		w.l.Lock()
		defer w.l.Unlock()
		return w.pubClient.Publish(context.Background(), w.options.Channel, &MSG{"UpdateForAddPolicy",
			w.options.LocalID,
			sec,
			ptype,
			fmt.Sprintf("%s %s", p1, p2)}).Err()
	})
}

// Update publishes a message to all other casbin instances telling them to
// invoke their update callback
func (w *WatcherEx1) Update() error {
	return w.logRecord(func() error {
		w.l.Lock()
		defer w.l.Unlock()
...
func (w *WatcherEx1) UpdateForAddPolicy(sec, ptype string, params ...string) error {
	return w.logRecord(func() error {
		w.l.Lock()
		defer w.l.Unlock()
...
func (w *WatcherEx1) UpdateForRemovePolicy(sec, ptype string, params ...string) error {
	return w.logRecord(func() error {
		w.l.Lock()
		defer w.l.Unlock()
...
func (w *WatcherEx1) UpdateForRemoveFilteredPolicy(sec, ptype string, fieldIndex int, fieldValues ...string) error {
	return w.logRecord(func() error {
		w.l.Lock()
		defer w.l.Unlock()
...
// It is called after Enforcer.RemoveFilteredNamedGroupingPolicy()
func (w *WatcherEx1) UpdateForSavePolicy(model model.Model) error {
	return w.logRecord(func() error {
		w.l.Lock()
		defer w.l.Unlock()
...
func (w *WatcherEx1)UpdateForAddPolicies(sec string, ptype string, rules ...[]string) error{
	return nil
}

func (w *WatcherEx1)UpdateForRemovePolicies(sec string, ptype string, rules ...[]string) error{
	return nil
}

func (w *WatcherEx1) logRecord(f func() error) error {
	err := f()
	if err != nil {
		log.Println(err)
	}
...
func (w *WatcherEx1) unsubscribe(psc *rds.PubSub) error {
	return psc.Unsubscribe(w.ctx)
}

func (w *WatcherEx1) subscribe() {
	w.l.Lock()
...
func (w *WatcherEx1) GetWatcherOptions() WatcherOptions {
	w.l.Lock()
	defer w.l.Unlock()
	return w.options
}

func (w *WatcherEx1) Close() {
	w.l.Lock()
	defer w.l.Unlock()
	close(w.close)
